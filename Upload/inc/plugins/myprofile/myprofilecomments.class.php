...
/* modcp */
$plugins->add_hook("modcp_nav", array(MyProfileComments::get_instance(), "modcp_nav"));
...
class MyProfileComments {
	...
	public function install() {
		...
		$db->write_query("ALTER TABLE `" . TABLE_PREFIX . "usergroups` ADD `caneditowncomments` int(1) NOT NULL default '0';");
		$db->write_query("ALTER TABLE `" . TABLE_PREFIX . "usergroups` ADD `candeleteowncomments` int(1) NOT NULL default '0';");
		
		// Give access to send and edit own comments to ALL USERGROUPS
		$db->write_query("UPDATE " . TABLE_PREFIX . "usergroups SET canmanagecomments = '0', cansendcomments = '1', caneditowncomments = '1'");
		// Give access to EDIT & DELETE (all comments) to Administrators
		$db->write_query("UPDATE " . TABLE_PREFIX . "usergroups SET canmanagecomments = '1', candeleteowncomments = '1' WHERE gid = '4'");
		// Delete all access for guests, banned users and awaiting activation
		$db->write_query("UPDATE " . TABLE_PREFIX . "usergroups SET cansendcomments = '0', caneditowncomments = '0' WHERE gid IN ('1', '5', '7')");
		
		$cache->update_usergroups();
		
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` ADD `mpcommentsopen` int(10) NOT NULL default '1';");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` ADD `mpcommentnotification` int(10) NOT NULL default '1';");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` ADD `mpwhocancomment` int(10) NOT NULL default '2';"); // 0=no one, 1=friendlist, 2=everyone
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` ADD `mpnewcomments` int(10) NOT NULL default '0';");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` ADD `mpcommentsapprove` int(10) NOT NULL default '0';");
	}
	...
	public function uninstall() {
		global $db, $cache;
		$db->write_query("DROP TABLE IF EXISTS " . TABLE_PREFIX . "myprofilecomments");
		...
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."usergroups` DROP `caneditowncomments`;");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."usergroups` DROP `candeleteowncomments`;");
		
		$cache->update_usergroups();
		
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` DROP `mpcommentsopen`;");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` DROP `mpcommentnotification`;");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` DROP `mpwhocancomment`;");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` DROP `mpnewcomments`;");
		$db->write_query("ALTER TABLE `".TABLE_PREFIX."users` DROP `mpcommentsapprove`;");
	}
...
<button type="button" class="button comment-action comment-delete-all">{$lang->mp_comments_action_delete_all_comments}</button>
...
<option value="1" {$comment_private_selected}>{$lang->mp_comments_comment_private}</option>
		...
		$templates["myprofile_comments_table"] = '<br />
<table border="0" cellspacing="{$theme[\'borderwidth\']}" cellpadding="{$theme[\'tablespace\']}" class="tborder">
<tr>
...
<fieldset class="trow2">';
		...
		$templates["myprofile_modcp_nav_comments"] = '<tr><td class="trow1 smalltext"><a href="modcp.php?action=myprofile" class="modcp_nav_item modcp_nav_ipsearch">{$lang->mp_myprofile_comments}</a></td></tr>';
...
<div class="float_right"><a href="#" id="mp_comments_notice_url" title="Dismiss this notice"><img src="http://localhost/mybb_1800/images/dismiss_notice.png" alt="Dismiss this notice" title="Dismiss this notice"></a></div>
...
60=60",
	...
	public function global_start() {
		global $mybb, $db, $settings, $lang, $templates, $myprofile_alertbar;
		/* if we are in member.php, and there is logged in user, or the user is viewing own profile */
		if(defined('THIS_SCRIPT') && THIS_SCRIPT == "member.php" && isset($mybb->user["uid"], $mybb->input["uid"], $mybb->input["action"]) && $mybb->input["action"] == "profile" && $mybb->user["uid"] > 0 && $mybb->input["uid"] == $mybb->user["uid"]) {
		...
		if($mybb->user["uid"] > 0 && $mybb->user["mpcommentnotification"] == "1" && $mybb->user["mpnewcomments"] > 0 && in_array($settings["mpcommentsnotification"], array("myalertsoralertbar", "alertbar"))) {
			/* we will only keep going on if the admin selected "Alert bar" or MyAlerts isn't installed */
	...
	public function xmlhttp_comments_add() {
		...
		if(strlen(trim($message)) < $settings["mpcommentsminlength"] || strlen($message) > $settings["mpcommentsmaxlength"]) {
			$result->error = true;
	...
	public function comment_process($comment, $memprofile) {
		...
		list($avatar_src, $avatar_width_height) = array_values(format_avatar($comment["avatar"]));
	...
	public function xmlhttp_comments_delete_all() {
		...
		if(! isset($mybb->input["memuberid"]) || ! is_numeric($mybb->input["memuberid"])) {
			$result->error = true;
	...
	public function modcp_reports_report() {
		...
		if($report["type"] == "comment") {
			...
			$from_profile_link = build_profile_link($from_user["username"], $from_user["uid"]);
			$to_profile_link = build_profile_link($to_user["username"], $to_user["uid"]);
	...
	public function build_comment_link($cid, $href=false, $link_name="", $other_params="") {
		...
		$complement = "#comments/" . ceil($comment["rownum"] / $settings["mpcommentsperpage"]) . "/" . $cid;
